...
本代码实现了一个完整的供应链库存管理仿真系统，通过对比三种不同的DQN算法
来研究深度强化学习在复杂供应链决策问题中的应用效果。

## 核心设计理念

1. **模块化设计**：
   - 环境类(Env)：模拟供应链动态
   - 智能体类(Agent)：实现不同的DQN算法
   - 训练函数：支持单智能体和多智能体训练
   - 可视化函数：全面的结果分析和对比

2. **三种算法对比**：
   - 基线DQN：标准的深度Q网络，作为性能基线
   - Double DQN：改进算法，解决Q值过估计问题
   - IDQN多智能体：独立学习的多智能体系统

3. **供应链建模**：
   - 三级供应链：制造商→分销商→零售商→客户
   - 状态空间：订单量、满足需求、库存水平
   - 动作空间：离散化的订单量决策
   - 奖励函数：基于利润最大化的实际业务逻辑
...
├── 智能体算法
│   ├── DQNAgent                     # 基线DQN智能体
│   └── DoubleDQNAgent              # 改进的Double DQN
├── 训练系统
│   ├── train_single_agent()         # 单智能体训练
│   └── train_marl()                 # 多智能体训练
├── 测试系统
│   ├── test_agent_comprehensive()   # 单智能体综合测试
│   └── test_marl_comprehensive()    # 多智能体综合测试
├── 可视化系统
│   ├── plot_learning_curves()       # 学习曲线对比
│   ├── plot_policy_comparison()     # 策略行为对比
│   └── plot_comprehensive_results() # 综合性能分析
└── 主执行流程
    └── 完整的三组对比实验
```
...
1. **算法性能对比**：验证Double DQN的改进效果
2. **牛鞭效应观察**：多智能体系统中的涌现现象
3. **结构性洞察**：供应链不同位置的战略优劣势

## 使用说明

直接运行本文件将自动执行完整的对比实验：
1. 训练三种不同的算法（每种2000回合）
2. 生成学习曲线和策略对比图
3. 进行综合性能测试和可视化分析
4. 保存训练好的模型和结果图表
